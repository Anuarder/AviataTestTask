    ...
    getters: {
        GET_FLIGHTS(state) {
            const timeToString = (sec) => {
                ...
                const hours = '' + Math.floor((sec - days * day_sec) / hour_sec)
                const minutes = '0' + Math.round((sec - days * day_sec - hours * hour_sec) / 60)
                return (`${days > 0 ? days + ' д ' : ''}${ hours.substr(-2)} ч ${minutes.substr(-2) != '00' ? minutes.substr(-2) + ' м' : ''}`)
            }
            ...
            for(let flight of state.flights){
                ...
                const last_segment = itinerary.segments[itinerary.segments.length - 1];
                ...
                const diffDay = diffInDays(first_segment.dep_time_iso, last_segment.arr_time_iso);
    ...
    actions: {
        async GET_DATA({ commit }) {
            ...
            try {
                ...
                ApiResult.flights.forEach(element => {
